...
import { CustomError, type UpdateUserDto } from "../../domain";
import type { HashAdapter } from "../../domain/interfaces/hash.adapter";
			...
			const enrichedUsers = await Promise.all(
				users.map(async (user) => {
					const role = user.role_id
						? await RoleModel.findById(user.role_id)
						: null;
	...
	async updateUser(
		...
		try {
			...
			if (updateUserDto.email !== undefined) {
				const existingEmailUser = await ProfileModel.findByEmail(
					updateUserDto.email,
				);
			...
			if (updateUserDto.password !== undefined) {
				updateData.password_hash = await this.hashAdapter.hash(
				...
				if (updateUserDto.role !== undefined) {
					...
					if (!role || !role.id) {
						throw CustomError.badRequest(
							`Role "${updateUserDto.role}" not found. Valid roles: admin, agent`,
						);
					}
			...
			const role = updatedUser.role_id
				? await RoleModel.findById(updatedUser.role_id)
				: null;
