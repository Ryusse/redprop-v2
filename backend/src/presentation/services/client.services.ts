...
import type { PropertyRow } from "../../domain/interfaces/database-rows";
import type {
	CurrencyInfo,
...
export class ClientServices {
	async listClients(filters?: {
		...
		const enrichedClients = await Promise.all(
			clients.map(async (client) => {
				...
				const properties = await this.enrichClientWithProperties(
					clientEntity.id,
					categoryName,
				);
	...
	private async enrichPropertyDetails(
		property: PropertyRow,
	): Promise<PropertyDetails> {
		const { PropertyPriceModel } = await import(
			"../../data/postgres/models/properties/property-price.model"
		);
		const { PropertyMultimediaModel } = await import(
			"../../data/postgres/models/properties/property-multimedia.model"
		);
		...
		const { PropertyAgeModel } = await import(
			"../../data/postgres/models/properties/property-age.model"
		);
		...
		if (propertyAddresses.length > 0) {
			const address = await AddressModel.findById(
				propertyAddresses[0].address_id,
			);
			if (address) {
	...
	private async enrichClientWithProperties(
		...
		const { PropertyModel } = await import(
			"../../data/postgres/models/properties/property.model"
		);
		const { PropertyTypeModel } = await import(
			"../../data/postgres/models/properties/property-type.model"
		);
		const { PropertyStatusModel } = await import(
			"../../data/postgres/models/properties/property-status.model"
		);
		const { ClientRentalModel } = await import(
			"../../data/postgres/models/rentals/client-rental.model"
		);
		const { RentalModel } = await import(
			"../../data/postgres/models/rentals/rental.model"
		);
		const { CurrencyTypeModel } = await import(
			"../../data/postgres/models/payments/currency-type.model"
		);
		...
		if (categoryName === "Lead") {
			const interests =
				await ClientPropertyInterestModel.findByClientId(clientId);
			...
			if (interests.length > 0) {
				...
				propertiesOfInterest = interestResults.filter(
		...
		if (categoryName === "Inquilino") {
			...
			if (clientRentals.length > 0) {
				const sortedRentals = clientRentals.sort((a, b) => {
					const dateA = a.contract_start_date
						? new Date(a.contract_start_date).getTime()
						: 0;
					const dateB = b.contract_start_date
						? new Date(b.contract_start_date).getTime()
						: 0;
					return dateB - dateA;
				});
				...
				if (activeRental?.property_id) {
					const property = await PropertyModel.findById(
						activeRental.property_id,
					);
					if (property) {
						const [propertyType, propertyStatus, enrichedDetails] =
							await Promise.all([
						...
						const rentals = await RentalModel.findAll({
							property_id: activeRental.property_id,
						});
						const rentalInfo = rentals.find(
							(r) => r.client_rental_id === activeRental.id,
						);

						let currency = null;
						if (rentalInfo?.currency_type_id) {
							currency = await CurrencyTypeModel.findById(
								rentalInfo.currency_type_id,
							);
						}
	...
	private async getClientConsultations(
		...
		const { ConsultationTypeModel } = await import(
			"../../data/postgres/models/crm/consultation-type.model"
		);

		const consultations =
			await ClientConsultationModel.findByClientId(clientId);
		...
		const enrichedConsultations = await Promise.all(
			consultations.map(async (consultation) => {
				const consultationType = await ConsultationTypeModel.findById(
					consultation.consultation_type_id,
				);

				let property = null;
				if (consultation.property_id) {
					const propertyData = await PropertyModel.findById(
						consultation.property_id,
					);
					if (propertyData) {
						property = {
							id: propertyData.id,
	...
	async addOwnedProperty(clientId: number, propertyId: number) {
		...
		const category = await ContactCategoryModel.findById(
			client.contact_category_id,
		);

		if (!category || category.name !== "Propietario") {
			throw CustomError.badRequest(
				"Client must be an Owner to associate properties",
			);
		}
		...
		if (!updatedProperty) {
			throw CustomError.internalServerError(
				"Failed to associate property with owner",
			);
		}
