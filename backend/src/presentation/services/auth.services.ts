...
export class AuthServices {
	...
	public async registerProfile(registerProfileDto: RegisterProfileDto) {
		const existProfile = await ProfileModel.findByEmail(
		...
		try {
				...
				if (!defaultRole || !defaultRole.id) {
					...
					if (!adminRole || !adminRole.id) {
						throw CustomError.internalServerError(
							'Default role "agent" or "admin" not found. Run: npm run db:seed-roles',
						);
					}
			...
			throw CustomError.internalServerError(
				`Error registering profile: ${error}`,
			);
		}
	}

	public async loginProfile(loginProfileDto: LoginProfileDto) {
		const profile = await ProfileModel.findByEmailWithPassword(
		...
		const isMatch = await this.hashAdapter.compare(
