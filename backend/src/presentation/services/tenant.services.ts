	...
	async createTenantWithProperty(
		...
		return await TransactionHelper.executeInTransaction(async () => {
			...
			if (createTenantDto.property_id) {
				...
				const activeRental =
					await RentalModel.findActiveByPropertyId(propertyId);
				if (activeRental) {
			...
			if (propertyId && createTenantDto.contract_start_date && newClient.id) {
				...
				if (
					...
					rental = await RentalModel.create({
						...
						start_date: normalizedStartDate || new Date(),
			...
			if (newClient.id) {
				const interests = await ClientPropertyInterestModel.findByClientId(
					newClient.id,
				);
				if (interests.length > 0) {
						...
						interests.map(async (interest) => {
							...
							return {
								...
								property_type: propertyType?.id
									? { id: propertyType.id, name: propertyType.name }
									: null,
								property_status: propertyStatus?.id
									? { id: propertyStatus.id, name: propertyStatus.name }
									: null,
					...
					propertiesOfInterest = properties.filter(
