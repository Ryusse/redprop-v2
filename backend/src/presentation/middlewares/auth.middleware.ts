...
import { CustomError, type JwtAdapter } from "../../domain";
...
export class AuthMiddleware {
	...
	requireAdminOrOwner = async (
		...
		try {
			...
			if (Number.isNaN(resourceIdNumber)) {
				return res.status(400).json({
			...
			if (!isAdmin && !isOwner) {
				return res.status(403).json({
					message:
						"Access denied. You can only edit your own profile or must be an admin",
				});
			}
