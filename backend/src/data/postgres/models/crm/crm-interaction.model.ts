...
export class CrmInteractionModel {
	...
	static async create(
		interactionData: CreateCrmInteractionDto,
	): Promise<CrmInteraction> {
		const client = PostgresDatabase.getClient();

		const query = `
            INSERT INTO ${CrmInteractionModel.TABLE_NAME} (
	...
	static async findById(id: number): Promise<CrmInteraction | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${CrmInteractionModel.TABLE_NAME} WHERE id = $1`;
		const result = await client.query(query, [id]);
		return result.rows[0] || null;
	}

	static async findAll(
		filters?: CrmInteractionFilters,
	): Promise<CrmInteraction[]> {
		const client = PostgresDatabase.getClient();
		let query = `SELECT * FROM ${CrmInteractionModel.TABLE_NAME}`;
	...
	static async update(
		id: number,
		updateData: Partial<CreateCrmInteractionDto>,
		...
		if (fields.length === 0) {
			return await CrmInteractionModel.findById(id);
		}

		values.push(id);

		const query = `
            UPDATE ${CrmInteractionModel.TABLE_NAME}
            SET ${fields.join(", ")}
            WHERE id = $${paramIndex}
            RETURNING *
        `;
	...
	static async delete(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${CrmInteractionModel.TABLE_NAME} WHERE id = $1`;
		const result = await client.query(query, [id]);
