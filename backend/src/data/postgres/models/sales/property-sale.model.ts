...
export class PropertySaleModel {
	...
	static async create(saleData: CreatePropertySaleDto): Promise<PropertySale> {
		...
		const query = `
            INSERT INTO ${PropertySaleModel.TABLE_NAME} (
	...
	static async findById(id: number): Promise<PropertySale | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${PropertySaleModel.TABLE_NAME} WHERE id = $1`;
	...
	static async findByPropertyId(
		...
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${PropertySaleModel.TABLE_NAME} WHERE property_id = $1 ORDER BY sale_date DESC LIMIT 1`;
	...
	static async findAll(filters?: PropertySaleFilters): Promise<PropertySale[]> {
		const client = PostgresDatabase.getClient();
		let query = `SELECT * FROM ${PropertySaleModel.TABLE_NAME}`;
	...
	static async update(
		id: number,
		updateData: Partial<CreatePropertySaleDto>,
	): Promise<PropertySale | null> {
		const client = PostgresDatabase.getClient();
		...
		if (fields.length === 0) {
			return await PropertySaleModel.findById(id);
		}

		values.push(id);

		const query = `
            UPDATE ${PropertySaleModel.TABLE_NAME}
            SET ${fields.join(", ")}
            WHERE id = $${paramIndex}
            RETURNING *
        `;
	...
	static async delete(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${PropertySaleModel.TABLE_NAME} WHERE id = $1`;
		const result = await client.query(query, [id]);
