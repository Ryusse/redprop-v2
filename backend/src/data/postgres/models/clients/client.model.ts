...
export class ClientModel {
	...
	static async create(clientData: CreateClientDto): Promise<Client> {
		...
		const query = `
            INSERT INTO ${ClientModel.TABLE_NAME} (
		...
		const values = [
			...
			clientData.purchase_interest !== undefined
				? clientData.purchase_interest
				: false,
			clientData.rental_interest !== undefined
				? clientData.rental_interest
				: false,
	...
	static async findById(id: number): Promise<Client | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${ClientModel.TABLE_NAME} WHERE id = $1 AND deleted = false`;
	...
	static async findByEmail(email: string): Promise<Client | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${ClientModel.TABLE_NAME} WHERE email = $1 AND deleted = false`;
	...
	static async findByDni(dni: string): Promise<Client | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${ClientModel.TABLE_NAME} WHERE dni = $1 AND deleted = false`;
	...
	static async findByPhone(phone: string): Promise<Client[]> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${ClientModel.TABLE_NAME} WHERE phone = $1 AND deleted = false`;
	...
	static async findAll(filters?: {
		...
		let query = `SELECT * FROM ${ClientModel.TABLE_NAME} WHERE deleted = false`;
	...
	static async delete(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `UPDATE ${ClientModel.TABLE_NAME} SET deleted = true WHERE id = $1 AND deleted = false RETURNING id`;
		const result = await client.query(query, [id]);
	...
	static async restore(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `UPDATE ${ClientModel.TABLE_NAME} SET deleted = false WHERE id = $1 AND deleted = true RETURNING id`;
		const result = await client.query(query, [id]);
		return (result.rowCount ?? 0) > 0;
	}
	...
	static async hardDelete(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${ClientModel.TABLE_NAME} WHERE id = $1`;
		const result = await client.query(query, [id]);
		return (result.rowCount ?? 0) > 0;
	}
}
