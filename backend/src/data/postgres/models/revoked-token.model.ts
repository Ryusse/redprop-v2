...
export class RevokedTokenModel {
	...
	static async create(data: CreateRevokedTokenDto): Promise<RevokedToken> {
		...
		const query = `
            INSERT INTO ${RevokedTokenModel.TABLE_NAME}
	...
	static async isRevoked(jti: string): Promise<boolean> {
		...
		const query = `
            SELECT 1 FROM ${RevokedTokenModel.TABLE_NAME}
            WHERE token_jti = $1
            AND expires_at > NOW()
            LIMIT 1
        `;
	...
	static async revokeAllUserTokens(
		...
		const query = `
            INSERT INTO ${RevokedTokenModel.TABLE_NAME} (token_jti, user_id, expires_at, reason)
            VALUES (
	...
	static async cleanExpiredTokens(): Promise<number> {
		...
		const query = `
            DELETE FROM ${RevokedTokenModel.TABLE_NAME}
	...
	static async getUserRevokedTokens(userId: number): Promise<RevokedToken[]> {
		...
		const query = `
            SELECT * FROM ${RevokedTokenModel.TABLE_NAME}
            WHERE user_id = $1
            AND expires_at > NOW()
	...
	static async getStats(): Promise<{
		...
		const query = `
            ...
            FROM ${RevokedTokenModel.TABLE_NAME}
        `;
