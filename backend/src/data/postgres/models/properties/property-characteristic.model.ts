...
export class PropertyCharacteristicModel {
	...
	static async create(
		characteristicData: CreatePropertyCharacteristicDto,
	): Promise<PropertyCharacteristic> {
		const client = PostgresDatabase.getClient();

		const query = `
            INSERT INTO ${PropertyCharacteristicModel.TABLE_NAME} (property_id, name, value)
            VALUES ($1, $2, $3)
            RETURNING *
        `;
	...
	static async findById(id: number): Promise<PropertyCharacteristic | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${PropertyCharacteristicModel.TABLE_NAME} WHERE id = $1`;
	...
	static async findByPropertyId(
		...
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${PropertyCharacteristicModel.TABLE_NAME} WHERE property_id = $1 ORDER BY name`;
	...
	static async update(
		id: number,
		updateData: Partial<CreatePropertyCharacteristicDto>,
		...
		if (fields.length === 0) {
			return await PropertyCharacteristicModel.findById(id);
		}

		values.push(id);

		const query = `
            UPDATE ${PropertyCharacteristicModel.TABLE_NAME}
            SET ${fields.join(", ")}
            WHERE id = $${paramIndex}
            RETURNING *
        `;
	...
	static async delete(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${PropertyCharacteristicModel.TABLE_NAME} WHERE id = $1`;
		const result = await client.query(query, [id]);
	...
	static async deleteByPropertyId(propertyId: number): Promise<number> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${PropertyCharacteristicModel.TABLE_NAME} WHERE property_id = $1`;
		const result = await client.query(query, [propertyId]);
