...
export class PropertyAddressModel {
	...
	static async create(
		data: CreatePropertyAddressDto,
	): Promise<PropertyAddress> {
		const client = PostgresDatabase.getClient();

		const query = `
            INSERT INTO ${PropertyAddressModel.TABLE_NAME} (property_id, address_id)
            VALUES ($1, $2)
	...
	static async findByPropertyId(
		...
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${PropertyAddressModel.TABLE_NAME} WHERE property_id = $1`;
	...
	static async findByAddressId(addressId: number): Promise<PropertyAddress[]> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${PropertyAddressModel.TABLE_NAME} WHERE address_id = $1`;
	...
	static async delete(propertyId: number, addressId: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${PropertyAddressModel.TABLE_NAME} WHERE property_id = $1 AND address_id = $2`;
	...
	static async deleteByPropertyId(propertyId: number): Promise<number> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${PropertyAddressModel.TABLE_NAME} WHERE property_id = $1`;
		const result = await client.query(query, [propertyId]);
