...
export class PropertyServiceModel {
	...
	static async create(
		data: CreatePropertyServiceDto,
	): Promise<PropertyService> {
		const client = PostgresDatabase.getClient();

		const query = `
            INSERT INTO ${PropertyServiceModel.TABLE_NAME} (property_id, service_id)
            VALUES ($1, $2)
	...
	static async findByPropertyId(
		...
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${PropertyServiceModel.TABLE_NAME} WHERE property_id = $1`;
	...
	static async findByServiceId(serviceId: number): Promise<PropertyService[]> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${PropertyServiceModel.TABLE_NAME} WHERE service_id = $1`;
	...
	static async delete(propertyId: number, serviceId: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${PropertyServiceModel.TABLE_NAME} WHERE property_id = $1 AND service_id = $2`;
	...
	static async deleteByPropertyId(propertyId: number): Promise<number> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${PropertyServiceModel.TABLE_NAME} WHERE property_id = $1`;
		const result = await client.query(query, [propertyId]);
