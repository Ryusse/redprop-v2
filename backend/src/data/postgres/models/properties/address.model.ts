...
export class AddressModel {
	...
	static async create(addressData: CreateAddressDto): Promise<Address> {
		...
		const query = `
            INSERT INTO ${AddressModel.TABLE_NAME} (
	...
	static async findById(id: number): Promise<Address | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${AddressModel.TABLE_NAME} WHERE id = $1`;
	...
	static async findByCityId(cityId: number): Promise<Address[]> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${AddressModel.TABLE_NAME} WHERE city_id = $1 ORDER BY full_address`;
	...
	static async findByStreetAndNumber(
		...
		const client = PostgresDatabase.getClient();
		let query = `SELECT * FROM ${AddressModel.TABLE_NAME} WHERE city_id = $1`;
	...
	static async update(
		...
		const query = `
            UPDATE ${AddressModel.TABLE_NAME}
            SET ${fields.join(", ")}
            WHERE id = $${paramIndex}
            RETURNING *
        `;
	...
	static async delete(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${AddressModel.TABLE_NAME} WHERE id = $1`;
		const result = await client.query(query, [id]);
