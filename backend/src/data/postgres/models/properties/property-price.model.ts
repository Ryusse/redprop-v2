...
export class PropertyPriceModel {
	...
	static async create(
		priceData: CreatePropertyPriceDto,
	): Promise<PropertyPrice> {
		const client = PostgresDatabase.getClient();

		const query = `
            INSERT INTO ${PropertyPriceModel.TABLE_NAME} (property_id, price, currency_type_id, operation_type_id, updated_at)
	...
	static async findById(id: number): Promise<PropertyPrice | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${PropertyPriceModel.TABLE_NAME} WHERE id = $1`;
	...
	static async findByPropertyId(propertyId: number): Promise<PropertyPrice[]> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${PropertyPriceModel.TABLE_NAME} WHERE property_id = $1 ORDER BY updated_at DESC`;
	...
	static async findCurrentByPropertyAndOperation(
		...
		const client = PostgresDatabase.getClient();
		const query = `
            SELECT * FROM ${PropertyPriceModel.TABLE_NAME}
	...
	static async update(
		id: number,
		updateData: Partial<CreatePropertyPriceDto>,
	): Promise<PropertyPrice | null> {
		const client = PostgresDatabase.getClient();
		...
		if (fields.length === 0) {
			return await PropertyPriceModel.findById(id);
		}
		...
		const query = `
            UPDATE ${PropertyPriceModel.TABLE_NAME}
            SET ${fields.join(", ")}
            WHERE id = $${paramIndex}
            RETURNING *
        `;
	...
	static async delete(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${PropertyPriceModel.TABLE_NAME} WHERE id = $1`;
		const result = await client.query(query, [id]);
	...
	static async deleteByPropertyId(propertyId: number): Promise<number> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${PropertyPriceModel.TABLE_NAME} WHERE property_id = $1`;
		const result = await client.query(query, [propertyId]);
