...
export class PropertyModel {
	...
	static async create(propertyData: CreatePropertyDto): Promise<Property> {
		...
		console.log(
			"[PropertyModel] propertyData.owner_id:",
			propertyData.owner_id,
			"type:",
			typeof propertyData.owner_id,
		);
		if (propertyData.owner_id !== undefined && propertyData.owner_id !== null) {
			console.log(
				"[PropertyModel] Adding owner_id to INSERT:",
				propertyData.owner_id,
			);
			...
			console.log(
				"[PropertyModel] owner_id is undefined or null, NOT adding to INSERT",
			);
		}
		...
		addOptionalField(
			"publication_date",
			propertyData.publication_date || new Date(),
		);
		addOptionalField(
			"featured_web",
			propertyData.featured_web !== undefined
				? propertyData.featured_web
				: false,
		);
		...
		addOptionalField(
			...
			propertyData.operation_commission_percentage,
		);
		addOptionalField(
			"producer_commission_percentage",
			propertyData.producer_commission_percentage,
		);
		...
		const query = `
            INSERT INTO ${PropertyModel.TABLE_NAME} (${fields.join(", ")})
	...
	static async findById(
		...
		const client = PostgresDatabase.getClient();
		let query = `SELECT * FROM ${PropertyModel.TABLE_NAME} WHERE id = $1`;
	...
	static async findByIdWithRelations(
		...
		const property = await PropertyModel.findById(id, includeArchived);
		...
		const propertyAddresses = await PropertyAddressModel.findByPropertyId(
			property.id,
		);
	...
	static async findAll(
		...
		let query = `
            ...
            FROM ${PropertyModel.TABLE_NAME} p
		...
		if (!filters?.includeArchived) {
			conditions.push(
				`p.visibility_status_id != (SELECT id FROM visibility_statuses WHERE name = 'Archivada')`,
			);
		}
	...
	static async update(
		...
		const query = `
            UPDATE ${PropertyModel.TABLE_NAME}
            SET ${fields.join(", ")}
            WHERE id = $${paramIndex}
            RETURNING *
        `;
	...
	static async archive(id: number): Promise<boolean> {
		...
		if (statusResult.rows.length === 0) {
			throw new Error(
				'Visibility status "Archivada" not found. Run: npm run db:seed-catalogs',
			);
		}
	...
	static async unarchive(
		...
		if (!visibilityStatusId) {
			...
			if (statusResult.rows.length === 0) {
				throw new Error(
					'Visibility status "Publicado" not found. Run: npm run db:seed-catalogs',
				);
			}
	...
	static async delete(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${PropertyModel.TABLE_NAME} WHERE id = $1`;
		const result = await client.query(query, [id]);
		return (result.rowCount ?? 0) > 0;
	}
}
