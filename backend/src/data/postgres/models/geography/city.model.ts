...
export class CityModel {
	...
	static async findAll(): Promise<City[]> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${CityModel.TABLE_NAME} ORDER BY name`;
	...
	static async findById(id: number): Promise<City | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${CityModel.TABLE_NAME} WHERE id = $1`;
	...
	static async findByProvinceId(provinceId: number): Promise<City[]> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${CityModel.TABLE_NAME} WHERE province_id = $1 ORDER BY name`;
		const result = await client.query(query, [provinceId]);
		return result.rows;
	}

	static async findByNameAndProvince(
		name: string,
		provinceId: number,
	): Promise<City | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${CityModel.TABLE_NAME} WHERE name = $1 AND province_id = $2`;
	...
	static async update(
		id: number,
		updateData: Partial<CreateCityDto>,
	): Promise<City | null> {
		const client = PostgresDatabase.getClient();
		const fields: string[] = [];
		...
		values.push(id);
		const query = `UPDATE ${CityModel.TABLE_NAME} SET ${fields.join(", ")} WHERE id = $${paramIndex} RETURNING *`;
	...
	static async delete(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${CityModel.TABLE_NAME} WHERE id = $1`;
		const result = await client.query(query, [id]);
