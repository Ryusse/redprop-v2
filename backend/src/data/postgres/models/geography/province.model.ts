...
export class ProvinceModel {
	...
	static async findAll(): Promise<Province[]> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${ProvinceModel.TABLE_NAME} ORDER BY name`;
	...
	static async findById(id: number): Promise<Province | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${ProvinceModel.TABLE_NAME} WHERE id = $1`;
	...
	static async findByCountryId(countryId: number): Promise<Province[]> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${ProvinceModel.TABLE_NAME} WHERE country_id = $1 ORDER BY name`;
	...
	static async findByName(name: string): Promise<Province | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${ProvinceModel.TABLE_NAME} WHERE name = $1`;
	...
	static async update(
		id: number,
		updateData: Partial<CreateProvinceDto>,
		...
		values.push(id);
		const query = `UPDATE ${ProvinceModel.TABLE_NAME} SET ${fields.join(", ")} WHERE id = $${paramIndex} RETURNING *`;
	...
	static async delete(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${ProvinceModel.TABLE_NAME} WHERE id = $1`;
		const result = await client.query(query, [id]);
