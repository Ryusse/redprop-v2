...
export class PropertyActivityModel {
	...
	static async create(
		activityData: CreatePropertyActivityDto,
	): Promise<PropertyActivity> {
		const client = PostgresDatabase.getClient();

		const query = `
            INSERT INTO ${PropertyActivityModel.TABLE_NAME} (
	...
	static async findById(id: number): Promise<PropertyActivity | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${PropertyActivityModel.TABLE_NAME} WHERE id = $1`;
	...
	static async findByPropertyId(
		...
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${PropertyActivityModel.TABLE_NAME} WHERE property_id = $1 ORDER BY activity_date DESC`;
		const result = await client.query(query, [propertyId]);
		return result.rows;
	}

	static async findAll(
		filters?: PropertyActivityFilters,
	): Promise<PropertyActivity[]> {
		const client = PostgresDatabase.getClient();
		let query = `SELECT * FROM ${PropertyActivityModel.TABLE_NAME}`;
	...
	static async delete(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${PropertyActivityModel.TABLE_NAME} WHERE id = $1`;
		const result = await client.query(query, [id]);
