...
export class PriceHistoryModel {
	...
	static async create(
		historyData: CreatePriceHistoryDto,
	): Promise<PriceHistory> {
		const client = PostgresDatabase.getClient();

		const query = `
            INSERT INTO ${PriceHistoryModel.TABLE_NAME} (
	...
	static async findById(id: number): Promise<PriceHistory | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${PriceHistoryModel.TABLE_NAME} WHERE id = $1`;
	...
	static async findByPropertyId(propertyId: number): Promise<PriceHistory[]> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${PriceHistoryModel.TABLE_NAME} WHERE property_id = $1 ORDER BY changed_at DESC`;
	...
	static async findByPropertyAndOperation(
		...
		const client = PostgresDatabase.getClient();
		const query = `
            SELECT * FROM ${PriceHistoryModel.TABLE_NAME}
	...
	static async delete(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${PriceHistoryModel.TABLE_NAME} WHERE id = $1`;
		const result = await client.query(query, [id]);
