...
export class AuditLogModel {
	...
	static async create(logData: CreateAuditLogDto): Promise<AuditLog> {
		...
		const query = `
            INSERT INTO ${AuditLogModel.TABLE_NAME} (
	...
	static async findById(id: number): Promise<AuditLog | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${AuditLogModel.TABLE_NAME} WHERE id = $1`;
	...
	static async findAll(filters?: AuditLogFilters): Promise<AuditLog[]> {
		const client = PostgresDatabase.getClient();
		let query = `SELECT * FROM ${AuditLogModel.TABLE_NAME}`;
	...
	static async delete(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${AuditLogModel.TABLE_NAME} WHERE id = $1`;
		const result = await client.query(query, [id]);
