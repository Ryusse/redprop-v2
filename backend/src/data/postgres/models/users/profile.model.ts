...
export class ProfileModel {
	...
	static async create(profileData: CreateProfileDto): Promise<Profile> {
		...
		const query = `
            INSERT INTO ${ProfileModel.TABLE_NAME} (first_name, last_name, email, password_hash, phone, role_id, active)
	...
	static async findByEmail(email: string): Promise<Profile | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${ProfileModel.TABLE_NAME} WHERE email = $1 AND deleted = false`;
	...
	static async findById(id: number | string): Promise<Profile | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${ProfileModel.TABLE_NAME} WHERE id = $1 AND deleted = false`;
	...
	static async update(
		...
		const query = `
            UPDATE ${ProfileModel.TABLE_NAME}
            SET ${fields.join(", ")}
	...
	static async findAll(): Promise<Profile[]> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${ProfileModel.TABLE_NAME} WHERE deleted = false ORDER BY created_at DESC`;
	...
	static async delete(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `UPDATE ${ProfileModel.TABLE_NAME} SET deleted = true WHERE id = $1`;
		const result = await client.query(query, [id]);
	...
	static async restore(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `UPDATE ${ProfileModel.TABLE_NAME} SET deleted = false WHERE id = $1`;
		const result = await client.query(query, [id]);
		return (result.rowCount ?? 0) > 0;
	}
	...
	static async hardDelete(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${ProfileModel.TABLE_NAME} WHERE id = $1`;
		const result = await client.query(query, [id]);
		return (result.rowCount ?? 0) > 0;
	}
