...
export class PaymentModel {
	...
	static async create(paymentData: CreatePaymentDto): Promise<Payment> {
		...
		const query = `
            INSERT INTO ${PaymentModel.TABLE_NAME} (
	...
	static async findById(id: number): Promise<Payment | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${PaymentModel.TABLE_NAME} WHERE id = $1`;
	...
	static async findByRentalId(rentalId: number): Promise<Payment[]> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${PaymentModel.TABLE_NAME} WHERE rental_id = $1 ORDER BY payment_date DESC`;
	...
	static async findAll(filters?: PaymentFilters): Promise<Payment[]> {
		const client = PostgresDatabase.getClient();
		let query = `SELECT * FROM ${PaymentModel.TABLE_NAME}`;
	...
	static async update(
		...
		const query = `
            UPDATE ${PaymentModel.TABLE_NAME}
            SET ${fields.join(", ")}
            WHERE id = $${paramIndex}
            RETURNING *
        `;
	...
	static async delete(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${PaymentModel.TABLE_NAME} WHERE id = $1`;
		const result = await client.query(query, [id]);
