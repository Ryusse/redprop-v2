...
export class RentalModel {
	...
	static async create(rentalData: CreateRentalDto): Promise<Rental> {
		...
		const query = `
            INSERT INTO ${RentalModel.TABLE_NAME} (
	...
	static async findById(id: number): Promise<Rental | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${RentalModel.TABLE_NAME} WHERE id = $1`;
	...
	static async findAll(filters?: RentalFilters): Promise<Rental[]> {
		const client = PostgresDatabase.getClient();
		let query = `SELECT * FROM ${RentalModel.TABLE_NAME}`;
	...
	static async findActiveByPropertyId(
		propertyId: number,
	): Promise<Rental | null> {
		const client = PostgresDatabase.getClient();
		const query = `
            SELECT * FROM ${RentalModel.TABLE_NAME}
	...
	static async update(
		id: number,
		updateData: Partial<CreateRentalDto>,
	): Promise<Rental | null> {
		const client = PostgresDatabase.getClient();
	...
	static async delete(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${RentalModel.TABLE_NAME} WHERE id = $1`;
		const result = await client.query(query, [id]);
