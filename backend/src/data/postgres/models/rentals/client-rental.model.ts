...
export class ClientRentalModel {
	...
	static async create(
		rentalData: CreateClientRentalDto,
	): Promise<ClientRental> {
		const client = PostgresDatabase.getClient();

		const query = `
            INSERT INTO ${ClientRentalModel.TABLE_NAME} (
		...
		const result = await client.query(query, [
			...
			rentalData.remind_increase !== undefined
				? rentalData.remind_increase
				: false,
			rentalData.remind_contract_end !== undefined
				? rentalData.remind_contract_end
				: false,
		]);
	...
	static async findById(id: number): Promise<ClientRental | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${ClientRentalModel.TABLE_NAME} WHERE id = $1`;
	...
	static async findByClientId(clientId: number): Promise<ClientRental[]> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${ClientRentalModel.TABLE_NAME} WHERE client_id = $1 ORDER BY contract_start_date DESC`;
	...
	static async findByPropertyId(propertyId: number): Promise<ClientRental[]> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${ClientRentalModel.TABLE_NAME} WHERE property_id = $1 ORDER BY contract_start_date DESC`;
	...
	static async update(
		id: number,
		updateData: Partial<CreateClientRentalDto>,
	): Promise<ClientRental | null> {
		const client = PostgresDatabase.getClient();
		...
		if (fields.length === 0) {
			return await ClientRentalModel.findById(id);
		}

		values.push(id);

		const query = `
            UPDATE ${ClientRentalModel.TABLE_NAME}
            SET ${fields.join(", ")}
            WHERE id = $${paramIndex}
            RETURNING *
        `;
	...
	static async delete(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${ClientRentalModel.TABLE_NAME} WHERE id = $1`;
		const result = await client.query(query, [id]);
