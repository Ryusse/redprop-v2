...
export class CurrencyTypeModel {
	...
	static async findAll(): Promise<CurrencyType[]> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${CurrencyTypeModel.TABLE_NAME} ORDER BY name`;
	...
	static async findById(id: number): Promise<CurrencyType | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${CurrencyTypeModel.TABLE_NAME} WHERE id = $1`;
	...
	static async findByName(name: string): Promise<CurrencyType | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${CurrencyTypeModel.TABLE_NAME} WHERE name = $1`;
	...
	static async findBySymbol(symbol: string): Promise<CurrencyType | null> {
		const client = PostgresDatabase.getClient();
		const query = `SELECT * FROM ${CurrencyTypeModel.TABLE_NAME} WHERE UPPER(symbol) = UPPER($1)`;
	...
	static async update(
		id: number,
		updateData: Partial<CreateCurrencyTypeDto>,
		...
		if (fields.length === 0) {
			return await CurrencyTypeModel.findById(id);
		}

		values.push(id);
		const query = `UPDATE ${CurrencyTypeModel.TABLE_NAME} SET ${fields.join(", ")} WHERE id = $${paramIndex} RETURNING *`;
	...
	static async delete(id: number): Promise<boolean> {
		const client = PostgresDatabase.getClient();
		const query = `DELETE FROM ${CurrencyTypeModel.TABLE_NAME} WHERE id = $1`;
		const result = await client.query(query, [id]);
