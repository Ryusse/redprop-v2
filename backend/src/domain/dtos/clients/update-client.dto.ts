...
export class UpdateClientDto {
	...
	static create(object: Record<string, unknown>): [string?, UpdateClientDto?] {
		...
		if (contact_category_id && Number.isNaN(Number(contact_category_id))) {
		...
		if (
			property_search_type_id &&
			Number.isNaN(Number(property_search_type_id))
		) {
		...
		if (
			...
			if (
				typeof property_interest_phone === "string" &&
				property_interest_phone.trim().length > 0
			) {
