export class CreatePropertyPriceDto {
	...
	static create(
		...
		const hasCurrencyId =
			currency_type_id !== undefined && currency_type_id !== null;
		const hasCurrencySymbol =
			(currency_symbol || currency) !== undefined &&
			(currency_symbol || currency) !== null;

		if (!hasCurrencyId && !hasCurrencySymbol) {
			return [
				"Currency type ID or currency symbol (ARS, USD, EUR) is required",
				undefined,
			];
		}
		...
		if (hasCurrencyId && hasCurrencySymbol) {
			return [
				"Provide either currency_type_id OR currency symbol, not both",
				undefined,
			];
		}
		...
		if (
			hasCurrencyId &&
			(Number.isNaN(Number(currency_type_id)) || Number(currency_type_id) <= 0)
		) {
		...
		if (
			hasCurrencySymbol &&
			typeof (currency_symbol || currency) !== "string"
		) {
			return [
				'Currency symbol must be a string (e.g., "ARS", "USD", "EUR")',
				undefined,
			];
		}

		const hasOperationId =
			operation_type_id !== undefined && operation_type_id !== null;
		const hasOperationName =
			operation_type !== undefined && operation_type !== null;

		if (!hasOperationId && !hasOperationName) {
			return [
				"Operation type ID or operation type name (Venta, Alquiler, etc.) is required",
				undefined,
			];
		}
		...
		if (hasOperationId && hasOperationName) {
			return [
				"Provide either operation_type_id OR operation_type name, not both",
				undefined,
			];
		}
		...
		if (
			hasOperationId &&
			(Number.isNaN(Number(operation_type_id)) ||
				Number(operation_type_id) <= 0)
		) {
		...
		if (hasOperationName && typeof operation_type !== "string") {
			return [
				'Operation type must be a string (e.g., "Venta", "Alquiler")',
				undefined,
			];
		}
		...
		return [
			...
			new CreatePropertyPriceDto(
				...
				hasCurrencySymbol
					? ((currency_symbol || currency) as string)
					: undefined,
