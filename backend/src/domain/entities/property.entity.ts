...
export class PropertyEntity {
	...
	static fromObject(object: Record<string, unknown>): PropertyEntity {
		...
		if (publication_date) {
			pubDate =
				publication_date instanceof Date
					? publication_date
					: new Date(publication_date as string | number);
			if (Number.isNaN(pubDate.getTime())) {
				throw CustomError.badRequest("Invalid publication date");
			}
		}
		...
		if (updated_at) {
			updatedDate =
				updated_at instanceof Date
					? updated_at
					: new Date(updated_at as string | number);
