...
export class ClientEntity {
	...
	static fromObject(object: Record<string, unknown>): ClientEntity {
		...
		if (!first_name || (first_name as string).trim().length === 0) {
			throw CustomError.badRequest("First name is required");
		}

		if ((first_name as string).trim().length < 2) {
		...
		if (!last_name || (last_name as string).trim().length === 0) {
			throw CustomError.badRequest("Last name is required");
		}

		if ((last_name as string).trim().length < 2) {
		...
		if (!phone || (phone as string).trim().length === 0) {
			throw CustomError.badRequest("Phone is required");
		}
		...
		if (email && (email as string).trim().length > 0) {
			if (!ClientEntity.isValidEmail(email as string)) {
				throw CustomError.badRequest("Invalid email format");
			}
		}

		if (!ClientEntity.isValidPhone(phone as string)) {
			throw CustomError.badRequest("Invalid phone format");
		}
		if (
			property_interest_phone &&
			(property_interest_phone as string).trim().length > 0
		) {
			if (!ClientEntity.isValidPhone(property_interest_phone as string)) {
				throw CustomError.badRequest("Invalid property interest phone format");
			}
		}

		if (dni && (dni as string).trim().length > 0) {
			if (!ClientEntity.isValidDni(dni as string)) {
		...
		return new ClientEntity(
			...
			dni ? (dni as string).trim() : undefined,
			property_interest_phone
				? (property_interest_phone as string).trim()
				: undefined,
			address ? (address as string).trim() : undefined,
			notes ? (notes as string).trim() : undefined,
			interest_zone ? (interest_zone as string).trim() : undefined,
			purchase_interest !== undefined ? Boolean(purchase_interest) : undefined,
			rental_interest !== undefined ? Boolean(rental_interest) : undefined,
			property_search_type_id !== undefined && property_search_type_id !== null
				? Number(property_search_type_id)
